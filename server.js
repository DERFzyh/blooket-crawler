...
async function injectCheat(gameMode, cheatType, value) {
  ...
  const c = {
    global: {
      turbo: `(function(){if(window.__aaId)clearInterval(window.__aaId);window.__aaLastQ='';window.__aaLastPw='';window.__aaId=setInterval(function(){try{function wF(n,d){if(!n||d>50)return null;try{var s=n.stateNode?.state;if(s)return s}catch(e){}return wF(n.child,d+1)||wF(n.sibling,d+1)||wF(n.return,d+1)}for(var el of document.querySelectorAll('*')){var k=Object.keys(el).filter(function(x){return x.indexOf('__react')===0});if(!k.length)continue;var s=wF(el[k[0]],0);if(!s)continue;var hasGame=Object.keys(s).some(function(k){return k==='question'||k==='stage'||k==='gold'||k==='choices'||k==='weight'||k==='lure'||k==='crypto'||k==='password'||k==='correctPassword'});if(!hasGame)continue;var qt=s.question&&(s.question.question||s.question.text);var weight=s.weight,lure=s.lure,pwOpts=s.passwordOptions;if(s.stage==='intro'&&pwOpts&&pwOpts.length>0&&!qt){document.querySelectorAll('div[class*="button"]').forEach(function(c){var t=(c.textContent||'').trim();if(t.length>2&&t.length<40&&c.offsetHeight>0&&c.offsetHeight<100&&!window.__aaLastPw){c.click();window.__aaLastPw=t}});}if(qt&&s.question.correctAnswers){var ca=Array.isArray(s.question.correctAnswers)?s.question.correctAnswers:[s.question.correctAnswers];document.querySelectorAll('[class*="answerContainer"]').forEach(function(c){var t=(c.textContent||'').trim();if(ca.some(function(a){return(a||'').toString().trim()===t})&&c.offsetHeight>0)c.click()})}if(s.stage==='prize'&&s.choices&&s.choices.length>=3){var bestVal=-1,bestIdx=0;s.choices.forEach(function(c,i){var txt=c.text||c.question||'';var m=txt.match(/\\+?\\s*(\\d+)/);var v=m?parseInt(m[1]):0;if(txt.indexOf('Triple')>=0)v=999;if(txt.indexOf('Double')>=0)v=666;if(v>bestVal){bestVal=v;bestIdx=i}});var ce=document.querySelectorAll('[class*="chest"],[class*="Chest"],[class*="prize"]');if(ce.length>bestIdx&&ce[bestIdx].offsetHeight>0)ce[bestIdx].click()}if((weight!==undefined||lure!==undefined)&&!qt){document.querySelectorAll('[class*="cast"],[class*="Cast"],[class*="fish"],[class*="Fish"],[class*="reel"],[class*="Reel"]').forEach(function(c){if(c.offsetHeight>0)c.click()})}if(pwOpts&&pwOpts.length>0&&!qt){var correctPw=s.correctPassword||(pwOpts[0]||'').toString().trim();var clicked=false;document.querySelectorAll('[class*="button"],[role=button]').forEach(function(c){var t=(c.textContent||'').trim();if(!clicked&&correctPw&&t===correctPw&&c.offsetHeight>0){c.click();clicked=true}});if(!clicked&&s&&s.password!==undefined&&s.passwordOptions){s.password=s.correctPassword||s.passwordOptions[0];if(s.forceUpdate)s.forceUpdate()}}document.querySelectorAll('button:not([disabled])').forEach(function(c){if(c.offsetHeight>0)c.click()});document.querySelectorAll('[role=button]').forEach(function(c){if(c.offsetHeight>0)c.click()});break}}catch(e){}},50);return'turbo-active: 50ms'})()`
    },
    gold: {
      setGold: `(function(){try{for(var e of document.querySelectorAll('*')){var k=Object.keys(e).find(function(x){return x.indexOf('__react')===0});if(!k)continue;function w(n,d){if(!n||d>50)return;try{var s=n.stateNode?.state;if(s&&s.gold!==undefined){s.gold=${value||9999};s.forceUpdate&&s.forceUpdate()}}catch(e){}w(n.child,d+1);w(n.sibling,d+1);w(n.return,d+1)}w(e[k],0)}}catch(e){}return'ok'})()`,
      pickSwap: `(function(){for(var e of document.querySelectorAll('*')){var k=Object.keys(e).find(function(x){return x.indexOf('__react')===0});if(!k)continue;function w(n,d){if(!n||d>50)return null;try{var s=n.stateNode?.state;if(s&&s.stage==='prize'&&s.choices&&s.choices.length>=3){var idx=s.choices.findIndex(function(c){var t=(c.text||c.question||'').toLowerCase();return t.indexOf('swap')>=0||t.indexOf('swp')>=0});if(idx>=0){var ce=document.querySelectorAll('[class*="chest"],[class*="Chest"],[class*="prize"]');if(ce.length>idx&&ce[idx].offsetHeight>0){ce[idx].click();return'picked-swap: #'+idx}}return'swap-not-found'}catch(e){}var r=w(n.child,d+1);if(r)return r;r=w(n.sibling,d+1);if(r)return r;return w(n.return,d+1)}var r=w(e[k],0);if(r)return r}return'no-prize-phase'})()`,
      pickTriple: `(function(){for(var e of document.querySelectorAll('*')){var k=Object.keys(e).find(function(x){return x.indexOf('__react')===0});if(!k)continue;function w(n,d){if(!n||d>50)return null;try{var s=n.stateNode?.state;if(s&&s.stage==='prize'&&s.choices&&s.choices.length>=3){var idx=s.choices.findIndex(function(c){var t=(c.text||c.question||'').toLowerCase();return t.indexOf('triple')>=0});if(idx>=0){var ce=document.querySelectorAll('[class*="chest"],[class*="Chest"],[class*="prize"]');if(ce.length>idx&&ce[idx].offsetHeight>0){ce[idx].click();return'picked-triple: #'+idx}}return'triple-not-found'}catch(e){}var r=w(n.child,d+1);if(r)return r;r=w(n.sibling,d+1);if(r)return r;return w(n.return,d+1)}var r=w(e[k],0);if(r)return r}return'no-prize-phase'})()`,
      pickDouble: `(function(){for(var e of document.querySelectorAll('*')){var k=Object.keys(e).find(function(x){return x.indexOf('__react')===0});if(!k)continue;function w(n,d){if(!n||d>50)return null;try{var s=n.stateNode?.state;if(s&&s.stage==='prize'&&s.choices&&s.choices.length>=3){var idx=s.choices.findIndex(function(c){var t=(c.text||c.question||'').toLowerCase();return t.indexOf('double')>=0});if(idx>=0){var ce=document.querySelectorAll('[class*="chest"],[class*="Chest"],[class*="prize"]');if(ce.length>idx&&ce[idx].offsetHeight>0){ce[idx].click();return'picked-double: #'+idx}}return'double-not-found'}catch(e){}var r=w(n.child,d+1);if(r)return r;r=w(n.sibling,d+1);if(r)return r;return w(n.return,d+1)}var r=w(e[k],0);if(r)return r}return'no-prize-phase'})()`,
      chestTarget: `(function(){if(window.__ctObserver)window.__ctObserver.disconnect();window.__ctObserver=new MutationObserver(function(){try{for(var e of document.querySelectorAll('*')){var k=Object.keys(e).find(function(x){return x.indexOf('__react')===0});if(!k)continue;function wf(n,d){if(!n||d>50)return null;try{var s=n.stateNode?.state;if(s&&s.stage==='prize'&&s.choices&&s.choices.length>=3)return s}catch(e){}return wf(n.child,d+1)||wf(n.sibling,d+1)||wf(n.return,d+1)}var s=wf(e[k],0);if(!s)continue;var t=window.__chestTarget||'Swap';var idx=-1;s.choices.forEach(function(c,i){var txt=(c.text||c.question||'').toLowerCase();if(t==='Swap'&&(txt.indexOf('swap')>=0||txt.indexOf('swp')>=0))idx=i;if(t==='Triple'&&txt.indexOf('triple')>=0)idx=i;if(t==='Double'&&txt.indexOf('double')>=0)idx=i;if(t==='Steal'&&(txt.indexOf('steal')>=0||txt.indexOf('stl')>=0))idx=i});if(idx>=0){var ce=document.querySelectorAll('[class*="chest"],[class*="Chest"],[class*="prize"]');if(ce.length>idx&&ce[idx].offsetHeight>0){ce[idx].click();console.log('[chestTarget] picked '+t+' #'+idx)}}}catch(e){}});window.__ctObserver.observe(document.body,{childList:true,subtree:true});window.__chestTarget='${value||'Swap'}';return'target:'+(window.__chestTarget)+' observer-active'})()`
    },
    fishing: {
...
io.on('connection', socket => {
  ...
  socket.on('cheat', async d => {
    // Handle esp (chest ESP) specially - injects GQ chest hack module
    if (d.cheatType === 'esp') {
      const r1 = await injectGQChestHack();
      const r2 = await gqGetChestInfo();
      const result = 'esp:' + r1 + ' chestInfo:' + JSON.stringify(r2).slice(0, 200);
      log(`🎯 [esp] → ${result}`);
      socket.emit('cheatResult', { cheatType: d.cheatType, result: result });
      return;
    }
