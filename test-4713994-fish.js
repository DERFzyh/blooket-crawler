...
(async()=>{
  ...
  try{const i=await p.$('input:not([hidden]):not([type=hidden])');if(i&&await i.isVisible()){await i.fill('FishTester')}}catch(e){}
  await p.keyboard.press('Enter');await p.waitForTimeout(8000);
  console.log('URL:',p.url());

  // Inject auto-answer (fixed: no break after answer)
  await p.evaluate(()=>{
    (function(){
      ...
      window.__aaId=setInterval(function(){
        try{
          ...
          for(var el of document.querySelectorAll('*')){
            ...
            // 1. Answer (NO break - falls through to step 5)
            ...
            if((weight!==undefined||lure!==undefined)&&!qt){
              document.querySelectorAll('[class*="fishingRod"],[class*="_fishingRod"],[class*="pageButton"],[class*="_pageButton"],[class*="cast"],[class*="reel"]').forEach(function(c){if(c.offsetHeight>0)c.click()});break;
            }
  ...
  let seen=new Set(),lastW=undefined;
  for(let t=0;t<30;t++){
    ...
    try{
      const r=await p.evaluate(()=>{
        ...
        for(var el of document.querySelectorAll('*')){
          ...
          var s=wF(el[k[0]],0);if(!s)continue;
          if(s.question||s.stage||s.gold!==undefined||s.weight!==undefined||s.lure!==undefined){
            return JSON.stringify({stage:s.stage,Q:s.question?(s.question.question||s.question.text):null,A:s.question?.correctAnswers?JSON.stringify(s.question.correctAnswers):null,gold:s.gold,weight:s.weight,lure:s.lure,isFrenzy:s.isFrenzy,url:location.href});
          }
        }
      ...
      if(d.Q&&!seen.has(d.Q)){
        ...
        console.log(`✅ ${d.A} | weight:${d.weight} gold:${d.gold} frenzy:${d.isFrenzy}`);
      } else if(d.weight!==undefined&&d.weight!==lastW){lastW=d.weight;console.log(`🐟 weight:${d.weight} frenzy:${d.isFrenzy} lure:${d.lure} gold:${d.gold}`)}
      else process.stdout.write('.');
    }catch(e){process.stdout.write('!')}
  }
  var stats=await p.evaluate(()=>window.__aaStats||{q:0}).catch(()=>({q:0}));
  console.log(`\n\n=== 📊 ===`);
