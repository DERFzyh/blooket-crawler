...
(async()=>{
  for(const PIN of ['5995773','6090638']){
    console.log('\n===',PIN,'===');
    ...
    for(let t=0;t<8;t++){
      ...
      const r=await p.evaluate(()=>{
        ...
        for(var el of document.querySelectorAll('*')){var k=Object.keys(el).filter(x=>x.indexOf('__react')===0);if(!k.length)continue;var s=wF(el[k[0]],0);
          if(s&&Object.keys(s).some(function(k){return['stage','question','weight','crypto','password'].indexOf(k)>=0}))
            return JSON.stringify({keys:Object.keys(s).slice(0,12),stage:s.stage,weight:s.weight,crypto:s.crypto,hasQ:!!s.question,hasPW:!!s.passwordOptions,path:location.pathname});
        }return JSON.stringify({url:location.href});
      });
      console.log('['+t+']',r.slice(0,200));
    }
